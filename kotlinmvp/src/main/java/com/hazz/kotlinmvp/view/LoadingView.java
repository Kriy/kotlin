...
/**
 * Created by Terminator on 2019/2/18.
 */
public class LoadingView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mOuterCircleRadius = (int) (Math.min(mOuterCircleRadius,
                (Math.min(w - getPaddingRight() - getPaddingLeft(), h - getPaddingTop() - getBottom()) - 4 * mPaint.getStrokeWidth() / 2)));
        ...
        mPath.lineTo(centerX - mInnerTriangleRadius / 2, (float) (centerX - Math.sqrt(3) * mInnerTriangleRadius / 2));
        mPath.close();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isReverse) {
            ...
            canvas.save();
            canvas.rotate(mRotageAngle, mRotateCenter.x, mRotateCenter.y);
